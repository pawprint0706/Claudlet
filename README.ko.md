...
Claude Code가 **서브에이전트**를 띄우면, 하나당 모자 쓴 작은 도우미가 펫을 졸졸
...
크리처의 포즈가 에이전트가 지금 뭘 하는지를 따라가요 — 편집·읽기·MCP 호출·생각·
입력 대기·완료(위 시트 참고). **auto/bypass 모드**에선 VR 바이저를 끼고 순항하고, 작업 종류별로
변형이 있어요. 또 **창에 올라타고 함께 다녀요** — 상단을 걷거나 안에서 지내요. 다른 창에 가려져도
잘리지 않고 펫과 컴패니언은 항상 온전히 보이며, 지내던 창이 최소화되거나 닫힐 때만 숨어요.

Claude Code가 **서브에이전트**를 돌리면 하나당 모자 쓴 **컴패니언**(최대 3마리)이 나타나 펫을
...
(아래 예시는 Claude Code 표기이며, Codex에서는 `$claudlet`으로 바꿔 쓰면 됩니다).
...
- **[설정](docs/configuration.ko.md)** — 어떤 에이전트 활동에 어떤 애니를 보일지 재매핑 (`claudlet-config` 또는 `/claudlet config`로 위치 확인·점검)
